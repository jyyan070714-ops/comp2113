...
1. Random Event Generation
   ...
   - Multiple unique event types are implemented: Ancient Shrine, Hidden Trap, Wandering Merchant, Campfire Gathering, Cursed Well, Elite Ghost.
...
2. Data Structures for Storing Data
   ...
   - Struct/class models: Card struct (card.h), Node struct (map.h), Player class (player.h/.cpp), Enemy struct/class (enemy.h/.cpp).
   - std::vector containers are used for flexible collections: allCards, rareCards, globalCardPool, player hand, map route.
   - Lookup and transformation helpers: getCardById, getUpgradedVersion, isRareCard (card.cpp), nodeTypeToString (map.cpp).
